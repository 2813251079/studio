            ...
            <CardContent className="flex flex-col items-center gap-6">
                <div className="relative" style={{ width: size, height: size }}>
                    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full">
                        ...
                        <circle cx={center} cy={center} r={center * 0.65} fill="hsl(var(--secondary))" stroke="hsl(var(--card))" strokeWidth="2" />
                        ...
                        {circleKeys.map((keyInfo, i) => {
                            ...
                            return (
                                <g key={`text-${keyInfo.major}`} className="pointer-events-none">
                                    ...
                                    <text x={majorCoords.x} y={majorCoords.y} textAnchor="middle" dominantBaseline="middle" className={cn("text-2xl font-bold", isSelected ? 'fill-primary' : 'fill-foreground' )}>{keyInfo.major}</text>
                                    <text x={minorCoords.x} y={minorCoords.y} textAnchor="middle" dominantBaseline="middle" className={cn("text-xl", isSelected ? 'fill-primary' : 'fill-foreground/80')}>{keyInfo.minor}</text>
                                </g>
                            )
                        })}
